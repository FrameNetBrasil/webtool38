    ...
    manager.action.prototype = {
        ...
        getAction: function (id, action) {
            ...
            if (modifier === ':') {
                ...
                var parts = action.substring(1).split('|');
                // var url = urlBase + "/index.php/" + parts[0];
                var url = urlBase + parts[0];
            ...
            if (modifier === 'd') {
                // action = manager.addAppToAction(action);
                return "manager.showDialog('" + action.substring(2) + "');";
            }
            if (modifier === 'p') {
                // action = manager.addAppToAction(action);
                return "manager.showPrompt('" + action.substring(2) + "');";
            }
            if (modifier === 'h') {
                // action = manager.addAppToAction(action);
                return "manager.showDialog('" + action.substring(2) + "');";
            ...
            if (modifier === 'f') {
                ...
                var url = urlBase + action.substring(2);
                console.log('****' +  url);
