    ...
    <div id="annotationStaticFrameMode2FETabs">
                ...
                <div title="{{$frame['name']}}" data-options="tools:'#tabTools_{{$idFrame}}'">
                    <form>
                        ...
                        <x-button class="ml-2 mb-2" id="btnSubmitFE{{$i}}" label="Submit FEs"
                                  hx-put="/annotation/staticFrameMode2/fes/{{$data->idStaticSentenceMM}}/{{$idFrame}}"></x-button>
                    </form>
                </div>
            @endforeach
        @else
        ...
        <script>
            $(function () {
                ...
                $('#annotationStaticFrameMode2FETabs').tabs('select', '{{$data->frames[$data->idFrame]['name']}}')
                @endif
            })
        </script>
    </div>
        ...
        @foreach($data->frames as $idFrame => $frame)
            <div id="tabTools_{{$idFrame}}">
                <a
                        ...
                        title="delete frame"
                        hx-delete="/annotation/staticFrameMode2/fes/{{$data->idStaticSentenceMM}}/{{$idFrame}}"
                        hx-target="body"
                ></a>
            </div>
        @endforeach
