...
class LayerTypeController extends MController
{
    ...
    public function newLayerType()
    {
        try {
            ...
            return $this->renderPrompt('information', 'OK', "structure.editEntry('{$this->data->layertype->entry}');");
        } catch (\Exception $e) {
    ...
    public function newGenericLabel()
    {
        try {
            ...
            if (!$model->exists($this->data->genericlabel)) {
                ...
                return $this->renderPrompt('information', 'Label created.');
            } else {
                return $this->renderPrompt('error', "Label already exists.");
            }
        } catch (\Exception $e) {
            return $this->renderPrompt('error', $e->getMessage());
        }
    ...
    public function updateGenericLabel()
    {
        try {
            ...
            $model->saveData($this->data->genericlabel);
            return $this->renderPrompt('information', 'Label updated.');
        } catch (\Exception $e) {
            return $this->renderPrompt('error', $e->getMessage());
        }
    }
    ...
    public function formDeleteGenericLabel()
    {
        ...
        return $this->renderPrompt('confirmation', 'Warning! The Label will be removed! Continue?', $ok);
    }
    ...
    public function deleteGenericLabel()
    {
        try {
            ...
            if (!$model->inUse()) {
                $model->delete();
                return $this->renderPrompt('information', 'Label deleted.', "!structure.reloadGenericLabel();");
            } else {
                return $this->renderPrompt('error', "Label is in use.");
            }
        } catch (\Exception $e) {
            return $this->renderPrompt('error', $e->getMessage());
        }
