...
use App\Services\Data\FrameService;
use Maestro\Manager;
...
//    }
    ...
    {
        $domain = new Domain();
        $this->data->domain = $domain->listForSelection()->getResult();
    ...
    public function grid() {
        ddump($this->data);
            if ($this->data->lu != '') {
                return $this->render("gridLU");
            } else if ($this->data->fe != '') {
                return $this->render("gridFE");
            } else {
                return $this->render("gridFrame");
            }
//        } elseif ($this->data->id[0] == 'f') {
//            $service = new FrameService();
//            return $service->listFELUforTree(substr($this->data->id, 1));
...
//        }
    ...
    public function edit(string $id) {
        if ($id[0] == 'f') {
            return $this->render("editFrame");
        } else if ($id[0] == 'e') {
            return $this->render("editFrameElement");
        } if ($id[0] == 'l') {
            return $this->render("editLU");
        } else {
            $this->notify("Error");
        }
    }
...
//    }
    ...
    public function newFrame()
    {
        try {
            ...
            return $this->renderResponse('ok', 'Frame created.');
        } catch (\Exception $e) {
            return $this->renderResponse('error', $e->getMessage());
        }
    }
    ...
    public function updateFrame()
    {
        try {
            ...
            return $this->renderResponse('ok', 'Frame updated.');
        } catch (\Exception $e) {
            return $this->renderResponse('error', $e->getMessage());
        }
    }
    ...
    public function formDeleteFrame()
    {
        ...
        return $this->renderPrompt('confirmation', 'Atenção: O Frame e todos os FrameElements serão removidos! Continua?', $ok);
    }
    ...
    public function deleteFrame()
    {
        try {
            ...
            return $this->renderResponse('information', 'OK', "!structure.reloadFrame();");
        } catch (\Exception $e) {
            return $this->renderResponse('error', $e->getMessage());
        }
...
//            $model->updateEntry($this->data->frameelement->entry);
    ...
    public function formDeleteFrameElement()
    {
        ...
        return $this->renderPrompt('confirmation', 'Warning: FrameElement will be removed! Continue?', $ok);
    }
    ...
    public function deleteFrameElement()
    {
        try {
            $model = new fnbr\models\FrameElement($this->data->id);
            $model->safeDelete();
            return $this->renderPrompt('information', 'FrameElement removed.', "!structure.reloadFrame();");
        } catch (\Exception $e) {
            return $this->renderPrompt('error', $e->getMessage());
        }
    }
    ...
    public function addConstraintFE()
    {
        try {
            ...
            return $this->renderPrompt('information', 'Constraint added.', "!structure.reloadFrameParent();");
        } catch (\Exception $e) {
            return $this->renderPrompt('error', "Add Constraint failed.");
        }
    }
    ...
    public function formDeleteConstraintFE()
    {
        ...
        if (!$hasChild) {
            ...
            return $this->renderPrompt('confirmation', 'Warning: Constraint will be deleted! Continue?', $ok);
        } else {
            return $this->renderPrompt('error', "This constraint has children; it can't be deleted!");
        }
    }
    ...
    public function deleteConstraintFE()
    {
        try {
            ...
            return $this->renderPrompt('information', 'Constraint deleted.', "!structure.reloadFrameParent();");
        } catch (\Exception $e) {
            return $this->renderPrompt('error', "Delete Constraint failed.", "!structure.reloadFrame();");
        }
    }
    ...
    public function newLU()
    {
        try {
            ...
            return $this->renderPrompt('information', 'OK, LU created; go to edition.', $updateLU . "$('#formNewLU_dialog').dialog('close');");
        } catch (\Exception $e) {
            return $this->renderPrompt('error', $e->getMessage());
        }
    }
    ...
    public function formDeleteLU()
    {
        ...
        return $this->renderPrompt('confirmation', 'Warning: LU will be removed! Continue?', $ok);
    }
    ...
    public function deleteLU()
    {
        try {
            $model = new fnbr\models\LU($this->data->id);
            $model->delete();
            return $this->renderPrompt('information', 'LU removed.', "!structure.reloadFrameParent();");
        } catch (\Exception $e) {
            return $this->renderPrompt('error', $e->getMessage());
        }
    }
    ...
    public function formDeleteSubCorpus()
    {
        ...
        return $this->renderPrompt('confirmation', 'Warning: SubCorpus will be removed! Continue?', $ok);
    }
    ...
    public function deleteSubCorpus()
    {
        try {
            ...
            if ($model->hasAnnotationSet()) {
                ...
                return $this->renderPrompt('confirmation', 'Warning: Related AnnotationSets will be removed! Continue?', $ok);
            } else {
                $model->delete();
                return $this->renderPrompt('information', 'SubCorpus removed.', "!structure.reloadFrameParent();");
            }
        } catch (\Exception $e) {
            return $this->renderPrompt('error', $e->getMessage());
        }
    ...
    public function addConstraintLU()
    {
        try {
            ...
            return $this->renderPrompt('information', 'Constraint added.');
        } catch (\Exception $e) {
            return $this->renderPrompt('error', "Add Constraint failed.");
        }
    }
    ...
    {
        if ($this->data->lemma == '') {
            return $this->renderPrompt('error', 'No lemma informed.');
        } elseif ($this->data->idLanguage == '') {
            return $this->renderPrompt('error', 'No language informed.');
        } else if (strpos($this->data->lemma, '.') === false) {
            return $this->renderPrompt('error', 'Wrong format for Lemma.');
        } else {
    ...
    public function newLemma()
    {
        try {
            ...
            return $this->renderPrompt('information', 'OK', "!$('#formNewLemma_dialog').dialog('close'); structure.reloadFrame();manager.doGet('{$url}','structureCenterPane');");
        } catch (\Exception $e) {
            return $this->renderPrompt('error', $e->getMessage());
        }
    }
    */
    ...
    public function formNewLexeme()
    {
        ...
        if ($this->data->lexeme == '') {
            return $this->renderPrompt('error', 'No lexeme informed.');
    ...
    public function newLexeme()
    {
        try {
            if ($this->data->lexeme->idPOS == '') {
                ...
                return $this->renderPrompt('information', 'OK', "!$('#formNewLexeme_dialog').dialog('close'); $('#gridLexema{$this->data->lexeme->name}').datagrid('reload');");
            }
        } catch (\Exception $e) {
            return $this->renderPrompt('error', $e->getMessage());
        }
    ...
    public function createTemplate()
    {
        try {
            ...
            return $this->renderPrompt('information', 'Template [' . $model->getName() . '] was created.');
        } catch (\Exception $e) {
            return $this->renderPrompt('error', $e->getMessage());
        }
    }
    ...
    public function newFrameRelations()
    {
        try {
            ...
            if ((count($relations['direct'])) || (count($relations['inverse']))) {
                return $this->renderPrompt('information', 'Frame created.', "structure.editRelations('{$this->data->frame->entry}');");
            } else {
                //return $this->renderPrompt('information', 'OK', "structure.editEntry('{$this->data->frame->entry}');");
                return $this->renderPrompt('information', 'OK', "structure.editEntry('{$this->data->frame->entry}','formNewFrame')");
                return $this->renderPrompt('information', 'OK');
            }
        } catch (\Exception $e) {
            return $this->renderPrompt('error', $e->getMessage());
