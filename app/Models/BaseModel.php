...
//    }
    ...
    static public function createEntity(string $type, string $prefix)
    {
        $entity = new EntityModel();
        do {
            $alias = $prefix . '_' . substr(uniqid(), -6);
            $entity->getByAlias($alias);
        } while (isset($entity->idEntity));
        $entity->alias = $alias;
        $entity->type = strtoupper($type);
        $entity->save();
        return $entity;
    }

    static public function createEntityRelation(int $idEntity1, string $relationTypeEntry, int $idEntity2, int $idEntity3 = null): ?int
    {
        $rt = new RelationTypeModel();
    ...
    static public function deleteAllEntityRelation(int $idEntity)
    {
        $er = new EntityRelationModel();
        $er->removeAllFromEntity($idEntity);
    }

    static public function createConstraintInstance(int $idConstraint, string $constraintTypeEntry, int $idConstrained, int $idConstrainedBy)
    {
        $ct = new ConstraintTypeModel();
        $ct->getByEntry($constraintTypeEntry);
        $ci = new ConstraintInstanceModel();
        $ci->saveData([
...
//    }
    ...
    {
        return MAuth::getLogin() ?? new UserModel(-1);
    }

    static public function getCurrentUserId()
    {
        return self::getCurrentUser()->getId() ?? 0;
    }

    static public function getCurrentUserLevel()
    {
        $userLevel = BaseModel::getCurrentUser()->getUserLevel();
        return $userLevel;
    }
